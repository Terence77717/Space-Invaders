...
Public Class Endless
    ...
    Dim moveddown As New List(Of String)
    'Dim enemyArray(maxEnemyNum) As PictureBox
    Dim enemyWaveList As New List(Of List(Of PictureBox))
    Dim enemyList As New List(Of PictureBox)
    Dim enemyListPosition As New List(Of List(Of Integer))
    Dim enemyOnScreen As New List(Of Boolean)
    Dim shooting As Boolean = False
    Dim enemyMoveRight As New List(Of String)
    ...
    Public Function insideAlien(k)
        If enemyList(k).Left < 0 Then
            enemyMoveRight(k) = "True"
        ElseIf enemyList(k).Left > 1220 Then
            enemyMoveRight(k) = "False"
        End If
    End Function
    ...
    Private Sub Form2_KeyPress(ByVal sender As Object, ByVal e As KeyEventArgs) Handles Me.KeyDown
        ...
        End Select
    End Sub
    Public Function AddNextWave()
        Dim wavelength = enemyWaveList.Count
        ...
        enemyWaveList.Add(modulefunc.enemywave)
        enemyListPosition.Add(modulefunc.enemywaveposition)
        enemyOnScreen = modulefunc.enemyOnScreen
        maxEnemyNum = maxEnemyNum + enemyWaveList(wavelength).Count

        moveddown.Add(False)
        enemyMoveRight.Add("True")
        tmrenemy.Start()
    End Function
    ...
    Dim totalrowsdown As Integer = 0
    Public Sub enemyMoveDirection()
        ...
        For wavecurrently = 0 To enemyWaveList.Count - 1
            For enemynum = 0 To enemyWaveList(wavecurrently).Count - 1
                If (enemyWaveList(wavecurrently)(enemynum).Left >= Me.Width - 60) Then
                    anyoutofborder = 1
                End If
                If (enemyWaveList(wavecurrently)(enemynum).Left <= 10) Then
                    anyoutofborder = 2
                End If
            Next
            If (anyoutofborder = 1) And (moveddown(wavecurrently) = "False") Then
                Debug.WriteLine("working")
                tmrenemy.Interval = 450
                amountchangetemporary = 20
                For i = 0 To enemyWaveList(wavecurrently).Count - 1
                    'Debug.WriteLine(enemyList(i).Left)
                    enemyWaveList(wavecurrently)(i).Top += 50
                    'enemyList(i).Left -= 15
                ...
                Next
                enemyMoveRight(wavecurrently) = "False"
                moveddown(wavecurrently) = "True"
                'Debug.WriteLine(moveddown)
                'Debug.WriteLine(enemyMoveRight)s
                amountchangetemporary = 20
                totalrowsdown = totalrowsdown + 1
            ElseIf (anyoutofborder = 2) And (moveddown(wavecurrently) = "False") Then
                For i = 0 To enemyWaveList(wavecurrently).Count - 1
                    enemyWaveList(wavecurrently)(i).Top += 50
                Next
                enemyMoveRight(wavecurrently) = "True"
                moveddown(wavecurrently) = "True"
                totalrowsdown = totalrowsdown + 1
            Else
    ...
    Private Sub tmrEnemy_Tick(sender As Object, e As EventArgs) Handles tmrenemy.Tick
        'enemyWaveList(0)(0).Left += 20
        'current = enemyList(0).Left
        For wavecurrent = 0 To enemyWaveList.Count - 1
            If enemyWaveList(wavecurrent).Count > 0 Then
                If enemyWaveList(wavecurrent)(enemyWaveList(wavecurrent).Count - 1).Top > (Me.Height - 2 * player.Height) Then
                    gamewon = "lost"
                    gameoverfunc()
                Else
                    enemyMoveDirection()
                    For i = 0 To enemyWaveList(wavecurrent).Count - 1
                        If enemyMoveRight(wavecurrent) = "True" Then
                            enemyWaveList(wavecurrent)(enemyWaveList(wavecurrent).Count - 1 - i).Left += 20
                        Else
                            enemyWaveList(wavecurrent)(i).Left = enemyWaveList(wavecurrent)(i).Left - amountchangetemporary 'current + ((enemyListPosition(i) Mod 10) * 70)
                            'enemyList(i).Left -= 20
                        End If
                    Next
                End If
